...
public class SmsSender {
  ...
  public static void sendSMS(String to, String smsContent) {
    if (AppPreference.isEclairOrLater()) {
      android.telephony.SmsManager.getDefault().sendTextMessage(to, null, smsContent, null, null);
    } else {
      android.telephony.gsm.SmsManager.getDefault().sendTextMessage(to, null, smsContent, null, null);
    }
  }
   ...
   */
  public void sendSMS(String phoneNumber, String action, Map<String, String> parameters, BroadcastReceiver onSent,
      BroadcastReceiver onDelivered) {
  ...
  public void sendSMS(String phoneNumber, String content, BroadcastReceiver onSent, BroadcastReceiver onDelivered) {
    PendingIntent sentPI = PendingIntent.getBroadcast(ctx, 0, new Intent(SENT_ACTION), 0);
    ctx.registerReceiver(onSent, new IntentFilter(SENT_ACTION));

    PendingIntent deliveredPI = PendingIntent.getBroadcast(ctx, 0, new Intent(DELIVERED_ACTION), 0);
    ctx.registerReceiver(onDelivered, new IntentFilter(DELIVERED_ACTION));

    if (AppPreference.isEclairOrLater()) {
      android.telephony.SmsManager.getDefault().sendTextMessage(phoneNumber, null, content, sentPI, deliveredPI);
    } else {
      android.telephony.gsm.SmsManager.getDefault().sendTextMessage(phoneNumber, null, content, sentPI, deliveredPI);
    }
