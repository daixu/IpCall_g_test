...
public class StrUitl {
  ...
  // 存放国标一级汉字不同读音的起始区位码
  private static int[] secPosValueList = { 1601, 1637, 1833, 2078, 2274, 2302, 2433, 2594, 2787, 3106, 3212, 3472,
      3635, 3722, 3730, 3858, 4027, 4086, 4390, 4558, 4684, 4925, 5249, 5600 };

  // 存放国标一级汉字不同读音的起始区位码对应读音
  private static char[] firstLetter = { 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q',
      'r', 's', 't', 'w', 'x', 'y', 'z' };

  private static String[] AZ = { "a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q",
      "r", "s", "t", "u", "v", "w", "x", "y", "z" };
  ...
  public static List<String> sort(List<String> lst) {
    ...
    for (int i = 0; i < l; i++) {
      for (j = 0; j < n; j++) {
        if (Character.toLowerCase(firstChar(lst.get(j))) >= Character.toLowerCase(firstChar(lst.get(j + 1)))) {
          temp = lst.get(j);
    ...
    for (int k = 0; k < l - 1; k++) {
      if (Character.toLowerCase(firstChar(lst.get(k))) == Character.toLowerCase(firstChar(lst.get(k + 1)))) {
        temp = lst.get(k);
