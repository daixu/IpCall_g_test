...
public class ChargeTask extends UserTask<Void, Void, String> implements BaseRequest.RequestListener {
  ...
  private void handleNormalResult() {
    new AlertDialog.Builder(mCtx).setTitle(mCtx.getString(R.string.recharge))
        .setMessage(mCtx.getString(R.string.recharge_suc)).setPositiveButton(mCtx.getString(R.string.ok), null).show();
  }
  ...
  private void handleExceptionResult(int resultCode) {
    ...
    mResultHandler.handle(resultCode, mChargeRequest.getReason(),
