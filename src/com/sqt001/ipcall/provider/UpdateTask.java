...
public class UpdateTask extends UserTask<Void, Void, String> implements BaseRequest.RequestListener {
  ...
  private void handleResult(String result) {
    ...
    if (isNew) {
      ...
      new AlertDialog.Builder(mCtx).setTitle(mCtx.getString(R.string.update_ver))
          .setMessage(mCtx.getString(R.string.is_newest_ver)).setPositiveButton(mCtx.getString(R.string.ok), null)
          .show();
    } else {
  ...
  private void handleUpdateResult() {
    new AlertDialog.Builder(mCtx).setTitle(R.string.new_version_hint).setMessage(AppPreference.getNewVersionMsg())
  ...
  private void download(String url) {
    if (!FileUtils.isSDCardReady()) {
      new AlertDialog.Builder(mCtx).setMessage(R.string.sdcard_not_ready).setPositiveButton(R.string.ok, null).show();
      return;
    }
  ...
  private void handleExceptionResult(int resultCode) {
    ...
    mResultHandler.handle(resultCode, mGetBalanceRequest.getReason(),
        new ExceptionResultHandler.ExceptionResultHandlerListener() {
          @Override
